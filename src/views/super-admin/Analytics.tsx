...
interface AnalyticsData {
  farmGrowthData: { month: string; farms: number }[];
  listingTrendsData: { month: string; created: number; removed: number }[];
  mostActiveFarmsData: { farm: string; listings: number }[];
  breedPopularityData: { breed: string; count: number }[];
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Breed Popularity */}
        {data.breedPopularityData.length > 0 && (
          <Card>
            ...
            <CardContent className="flex items-center justify-center">
              <ResponsiveContainer width="100%" height={280}>
                <PieChart>
                  ...
                  <Pie data={data.breedPopularityData} dataKey="count" nameKey="breed" cx="50%" cy="50%" innerRadius={55} outerRadius={100} paddingAngle={3} cornerRadius={6} stroke="#fff" strokeWidth={3} label={({ breed, percent }) => `${breed} ${(percent * 100).toFixed(0)}%`} animationDuration={1200} style={{ filter: 'drop-shadow(2px 4px 8px rgba(0,0,0,0.12))' }}>
                    {data.breedPopularityData.map((_, i) => <Cell key={i} fill={`url(#${PIE_GRADIENTS[i % PIE_GRADIENTS.length].id})`} />)}
                  </Pie>
        ...
        {/* Empty state */}
        {data.farmGrowthData.length === 0 && data.listingTrendsData.length === 0 && data.mostActiveFarmsData.length === 0 && data.breedPopularityData.length === 0 && (
          <Card className="col-span-2">
            <CardContent className="p-12 text-center text-muted-foreground">No analytics data available yet. Data will appear as farms and listings are created.</CardContent>
          </Card>
        )}
      </div>
    </div>
