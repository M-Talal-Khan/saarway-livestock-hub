...
import {
  ...
  Pin, Search, Factory, User, Heart, TrendingUp, Clock, Bookmark, Filter, Flame, Eye, Lock,
} from "lucide-react";
...
export default function ForumPage() {
  ...
  const handleNewPost = () => { router.push("/erp/forum/post/new"); };
          ...
          <div className="grid grid-cols-2 gap-2">
            {categories.slice(0, 6).map((cat, idx) => {
              ...
              return (
                <button
                  key={cat.id}
                  onClick={() => router.push(`/erp/forum/categories/${cat.slug}`)}
          ...
          <CardContent className="p-4">
            ...
            <div className="mt-3 space-y-1">
              {trendingPosts.slice(0, 5).map((post, idx) => (
                <Link key={post.id} href={`/erp/forum/post/${post.id}`}
      ...
      <Card key={post.id} className="group overflow-hidden border-sw-green-100 bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:border-sw-green-300 hover:shadow-md">
        <div className="flex items-start gap-4 p-4 sm:p-5">
          ...
          <div className="flex-1 min-w-0">
            <Link href={`/erp/forum/post/${post.id}`} className="block">
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl bg-gradient-to-br from-sw-green-900 via-sw-green-700 to-sw-green-500 p-5 sm:p-6 text-white shadow-[0_18px_45px_-25px_rgba(20,83,45,0.65)]">
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            <Badge className="mb-3 border-white/20 bg-white/15 text-white hover:bg-white/20">Farm community</Badge>
            <h1 className="text-2xl sm:text-3xl font-bold">Community Forum</h1>
            <p className="text-sm text-white/75 mt-2 max-w-2xl">Discuss livestock care, feed, sales, and farm operations with Saarway users.</p>
          </div>
