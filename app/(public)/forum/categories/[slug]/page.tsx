...
export default function PublicCategoryPage() {
  ...
  return (
    <div className="container mx-auto px-4 pb-8 pt-24 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <Button onClick={handleNewPost} className="gap-2 bg-sw-green-700 hover:bg-sw-green-800">
      ...
      {loading ? (
        ...
        <Card className="p-10 text-center border-dashed border-2">
          ...
          <Button className="mt-4 gap-2 bg-sw-green-700 hover:bg-sw-green-800" onClick={handleNewPost}>
          ...
          {posts.map(post => {
            ...
            return (
              <Card key={post.id} className="hover:shadow-md transition-all border-l-4 border-l-sw-green-500">
                <Link href={`/forum/post/${post.id}`} className="block p-4">
      ...
      <Dialog open={showLoginDialog} onOpenChange={setShowLoginDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              You need to be logged in to post a discussion.
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-3 mt-2">
            <Button onClick={() => { setShowLoginDialog(false); router.push("/login"); }} className="gap-2 bg-sw-green-700 hover:bg-sw-green-800">
              <LogIn className="w-4 h-4" /> Login as Individual User
            </Button>
            <Button variant="outline" onClick={() => { setShowLoginDialog(false); router.push("/farm-login"); }} className="gap-2">
              Login as Farm Staff
            </Button>
            <Button variant="ghost" onClick={() => setShowLoginDialog(false)} className="text-muted-foreground">
              Cancel
            </Button>
          </div>
        </DialogContent>
