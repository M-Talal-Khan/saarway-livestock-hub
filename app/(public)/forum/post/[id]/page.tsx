...
import { ArrowLeft, Heart, Bookmark, Pin, Lock, Edit, Trash2, MessageCircle, Send, Factory, User, X, Check, LogIn, Flame, Reply, Clock } from "lucide-react";
...
function CommentItem({ comment, replies, userId, userType, isAdmin, onEdit, onDelete, onSaveEdit, onCancelEdit, editingId, editText, setEditText, onReply, replyingToId, replyText, setReplyText, onCancelReply, onSubmitReply, submittingReplyId, depth = 0, isLocked, isLoggedIn }: {
  comment: any; replies: any[]; userId: string | null; userType: string; isAdmin: boolean; onEdit: (c: any) => void; onDelete: (id: string) => void;
  ...
  onCancelReply: () => void; onSubmitReply: (parentId: string) => void; submittingReplyId: string | null; depth?: number; isLocked: boolean; isLoggedIn: boolean;
}) {
        ...
        {replyingToId === comment.id && !isLocked && (
          <div className="mt-2 rounded-xl border border-sw-green-100 bg-sw-green-50/60 p-3">
            <textarea
              ...
              rows={2}
              placeholder={isLoggedIn ? `Reply to ${comment.author?.full_name ?? "this comment"}...` : "Login to reply..."}
              disabled={!isLoggedIn}
              className="w-full resize-none rounded-lg border border-input bg-white p-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-sw-green-500 disabled:opacity-60"
              autoFocus={isLoggedIn}
            />
            <div className="mt-2 flex justify-end gap-2">
              <Button size="sm" variant="ghost" onClick={onCancelReply} className="h-7 px-2 text-xs">Cancel</Button>
              <Button size="sm" onClick={() => onSubmitReply(comment.id)} disabled={!isLoggedIn || submittingReplyId === comment.id || !replyText.trim()} className="h-7 px-2 bg-sw-green-700 hover:bg-sw-green-800 text-xs">
...
export default function PublicPostDetailPage() {
  ...
  const handleSavePost = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isLoggedIn || !editPostTitle.trim() || !editPostContent.trim()) return;
    setSavingPost(true);
    const headers = await fetchAuthHeaders();
    const res = await fetch(`/api/erp/forum/posts/${id}`, {
      method: "PATCH",
          ...
          <CardContent className="p-4">
            ...
            <div className="flex flex-col divide-y divide-sw-green-100/50">
              {trendingPosts.map(tp => {
                ...
                return (
                  <Link key={tp.id} href={`/forum/post/${tp.id}`} className="block group">
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 pb-8 pt-24 max-w-6xl">
  ...
  return (
    <div className="container mx-auto px-4 pb-8 pt-24 max-w-6xl space-y-6">
      {/* Back */}
      <Link href="/forum" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-sw-green-700 transition-colors">
      ...
      <Card className="shadow-sm">
        <CardContent className="p-6 space-y-4">
          ...
          {!post?.is_locked ? (
            <form onSubmit={handleComment} className="space-y-3 pt-2 border-t">
              <Textarea
                placeholder={isLoggedIn ? "Write your reply..." : "Login to reply..."}
                rows={4}
                value={commentText}
              ...
              <div className="flex justify-end">
                {isLoggedIn ? (
                  ...
                  <Button type="button" onClick={() => router.push("/login")} className="gap-2 bg-sw-green-700 hover:bg-sw-green-800">
                    <LogIn className="w-4 h-4" /> Login to Reply
                  </Button>
                )}
              </div>
            </form>
        ...
        <div className="hidden lg:block w-72 xl:w-80 flex-shrink-0 sticky top-24">
          <SuggestedSidebar />
        </div>
      </div>
