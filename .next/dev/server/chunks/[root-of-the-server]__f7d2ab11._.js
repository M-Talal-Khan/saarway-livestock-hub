...
async function GET() {
    ...
    const [farmsRes, cattleRes, listingsRes] = await Promise.all([
        admin.from("farms").select("id, city", {
            count: "exact"
        }).eq("is_active", true),
        admin.from("cattle").select("breed", {
            count: "exact"
        }),
        admin.from("listings").select("id", {
            count: "exact"
        }).eq("status", "active")
    ]);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f7d2ab11._.js.map