...
async function GET(request, { params }) {
    ...
    const [farmRes, stationsRes, listingsRes] = await Promise.all([
        admin.from("farms").select("id, farm_name, city").eq("id", id).eq("is_active", true).single(),
        admin.from("stations").select("id, station_name, city").eq("farm_id", id).eq("is_active", true),
        admin.from("listings").select("id, asking_price, photo_url, listed_at, cattle(breed, current_weight, gender, teeth)").eq("farm_id", id).eq("status", "active").order("listed_at", {
            ascending: false
        })
    ]);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a24221c3._.js.map