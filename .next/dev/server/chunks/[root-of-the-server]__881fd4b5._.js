...
"[project]/app/api/marketplace/[id]/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request, { params }) {
    ...
    const { data: listing, error } = await admin.from("listings").select(`id, asking_price, photo_url, description, listed_at, farm_id,
       cattle(cattle_code, breed, teeth, current_weight, gender, coat_color),
       stations(station_name, station_tag, city),
       farms(id, farm_name, city)`).eq("id", id).eq("status", "active").single();
    if (error || !listing) return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        error: "Listing not found"
    }, {
        status: 404
    });
    // Fetch more listings from same farm
    const { data: moreListing } = await admin.from("listings").select("id, asking_price, photo_url, cattle(breed, current_weight)").eq("farm_id", listing.farm_id).eq("status", "active").neq("id", id).limit(3);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__881fd4b5._.js.map