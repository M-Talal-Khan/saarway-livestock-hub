...
"[project]/app/api/erp/cattle/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    ...
    let query = admin.from("cattle").select(`
    ...
    `).eq("farm_id", session.farm_id).order("created_at", {
        ascending: false
    });
    ...
    if (allIds.length > 0) {
        const { data: saleItems } = await admin.from("sale_items").select("cattle_id, sales(sale_type)").in("cattle_id", allIds);
        if (saleItems && saleItems.length > 0) {
            const cattleStatusMap = new Map((data ?? []).map((c)=>[
                    c.id,
                    c.status
                ]));
            const toMarkSold = [];
            const toMarkSlaughtered = [];
            ...
            if (toMarkSlaughtered.length > 0) {
                await admin.from("cattle").update({
                    status: "slaughtered"
                }).in("id", toMarkSlaughtered);
            }
            if (toMarkSold.length > 0 || toMarkSlaughtered.length > 0) {
                const fixedData = (data ?? []).map((c)=>{
                    if (toMarkSold.includes(c.id)) return {
                        ...c,
                        status: "sold"
                    };
                    if (toMarkSlaughtered.includes(c.id)) return {
                        ...c,
                        status: "slaughtered"
                    };
                    return c;
...
async function POST(request) {
    ...
    const { data, error } = await admin.from("cattle").insert({
        ...
        status: "active"
    }).select(`id, cattle_code, breed, teeth, gender, status, current_weight, purchase_price, purchase_date, station_id, stations(station_tag, station_name, city)`).single();
    if (error) return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        error: error.message
    }, {
        status: 500
...
//# sourceMappingURL=%5Broot-of-the-server%5D__98a3c7b7._.js.map